...
export class ManageProductsComponent extends BaseComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,injector: Injector,private route: ActivatedRoute, private router: Router, private httpclient: HttpClient, private orderSer: OrderService) {
    super(injector);
  ...
  ngOnInit(): void {
    ...
    this.orderSer.getTotalOrder().subscribe(
      (orders: any[]) => {
        this.totalOrder = orders.length;
        console.log(this.totalOrder)
      },
      (error) => {
        console.log('Hiệp ngu', error)
      }
    );
  }
  ...
  loadPage(page) { 
    this._api.post('/api/SanPham/search',{page: page, pageSize: this.pageSize}).takeUntil(this.unsubscribe).subscribe(res => {
      ...
      // console.log('đạt kiêm ngu như chó',this.products);
  ...
  //       sanpham.MoTa = form.controls['moTa'].value;
  //       sanpham.Anh = res.filePath;
  //       this._api.post('/api/SanPham/update-product', sanpham).takeUntil(this.unsubscribe).subscribe(res => {
  //         alert("Thêm mới thành công");
  //         this.resetform(form);
  //         this.search();
  //         this.displayAdd = false;

...
export class ManageOrdersComponent extends BaseComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, injector: Injector,private route: ActivatedRoute, private router: Router, private orderSer: OrderService, private http: HttpClient) {
    super(injector);
  }
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this._api.get('/api/DonHang/get-by-id/' + row.maDonHang).takeUntil(this.unsubscribe).subscribe((res: any) => {
        ...
        this._api.get('/api/SanPham/get-all').takeUntil(this.unsubscribe).subscribe((res: any) => {
          this.SanPhams = res;
          // console.log('đạt kiêm ngu như chó',this.SanPhams);
        });
        this.status =  [
